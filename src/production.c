...
bool production(int argc, char* argv[])
{
  ...
  */
    ...
    char shipSymbol = '\0';       /* temporary value to save character symbol of the ship */
        ...
        /* stream to output file battleship.log */

    /* Start of Program */
    FILE*  outStream = fopen("battleship_output.txt", "w");
    if (outStream == NULL)
    {
    ...
    {
        setvbuf (outStream, NULL, _IONBF, 0); //If we don't set this, we get some really strange buffering issues, and many things just don't print to the file
        ...
        puts("> Player 2 (Computer's) board has been generated.\n");
            ...
            switch (player) {
                ...
                case PLAYER_ONE:
                    ...
                    do {
                        ...
                        if (shot == -1)
                            puts("> Try inputting another target!\n");
